...
class TestSearchHN(unittest.TestCase):
    ...
    def test_get_user(self):
        # Get a user
        print(self.hn.user("nicolashahn").get())
        """
    ...
    def test_latest(self):
        ...
        latest_btc = self.hn.search("bitcoin").stories().latest()
        print(latest_btc)
        """
        SearchHN object:
    ...
    def test_whoishiring(self):
        # get all comments from the latest "Who is hiring" thread:
        whoishiring = self.hn.get_latest_whoishiring_thread().get_story_comments()
        python_jobs = [post for post in whoishiring if "python" in post.comment_text.lower()]

        print(
            "{} jobs available, {} python jobs available".format(
                len(whoishiring), len(python_jobs)
            )
        )
        """
